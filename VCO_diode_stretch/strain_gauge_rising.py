...
def MakeStretchGraph(dataframe):
    ...
    ax.errorbar(strain_1_avg.index / L0 * 100, strain_1_avg.values,
                yerr=strain_1_std.values, label="strain 1")
    ax.errorbar(strain_2_avg.index / L0 * 100, strain_2_avg.values,
                yerr=strain_2_std.values, label="strain 2")
    ax.errorbar(strain_3_avg.index / L0 * 100, strain_3_avg.values,
                yerr=strain_3_std.values, label="strain 3")
...
def angleStrainGraph(dataframe):
    ...
    color = 'tab:red'
    ax1.set_xlabel('strain as measured by the micrometer (%)')
    ax1.set_ylabel('angle', color=color)
    # ax1.plot(df['stretchAmt']*100, df['angle'], color=color)
    ax1.errorbar(angle_avg.index / L0 * 100, angle_avg.values,
                 yerr=angle_std.values, label="angle", color=color)
    ...
    ax2.set_ylabel('strain as measured by the strain gauges (%)', color=color)
    ax2.errorbar(strain_avg.index / L0 * 100, strain_avg.values *
                 100, yerr=strain_std.values, label="strain", color=color)
...
if __name__ == "__main__":
    ...
    df = helpers.importCSV(
        f"./VCO_diode_stretch/data/{meas}/rising/angle_{angle}.0.csv")
    ...
    fig.savefig(
        f"./VCO_diode_stretch/figures/{meas}/{angle}/strainGauges.png", dpi=500)

    fig2 = angleStrainGraph(df)
    fig2.savefig(
        f"./VCO_diode_stretch/figures/{meas}/{angle}/strainangle.png", dpi=500)
