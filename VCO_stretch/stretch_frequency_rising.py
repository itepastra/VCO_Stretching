...
def MakeStretchGraph1(dataframe):
    ...
    for lab, df in dataframe.groupby("stretchAmt"):
        ...
        ax.errorbar(df["Vctrl"], df["Freq1"]/1e6, yerr=df["errFreq1"]/1e6/sqrt(numberofsamples), capsize=None, lw=1, label=label)
...
def RelativeStretchGraph1(dataframe):
    ...
    for lab, df in dataframe.groupby("stretchAmt"):
        ...
        relErr = df["errFreq1"]/firstFreq/sqrt(numberofsamples)
...
def MakeStretchGraph2(dataframe):
    ...
    for lab, df in dataframe.groupby("stretchAmt"):
        label = f"{(lab/L0 * 100):.3f}% {RelativeToRadius(lab/L0) * 1000:.2f} mm"
        ax.errorbar(df["Vctrl"], df["Freq2"]/1e6, yerr=df["errFreq2"]/1e6/sqrt(numberofsamples), capsize=None, lw=1, label=label)
...
def RelativeStretchGraph2(dataframe):
    ...
    for lab, df in dataframe.groupby("stretchAmt"):
        ...
        # print(f"{lab} has relFreqs {relFreq}")
        relErr = df["errFreq2"]/firstFreq/sqrt(numberofsamples)
        # print(f"{lab} has relErr {relErr}")
        ax.errorbar(df["Vctrl"], relFreq, yerr=relErr, capsize=None, lw=1, label=label)
...
if __name__ == "__main__":
    ...
    fig.savefig(f"./VCO_stretch/figures/{meas}/stretch1_up_{angle}.png", dpi=500)
    fig2 = RelativeStretchGraph1(df)
    fig2.savefig(f"./VCO_stretch/figures/{meas}/stretch1_up_{angle}_rel.png", dpi=500)
    fig3 = MakeStretchGraph2(df)
    fig3.savefig(f"./VCO_stretch/figures/{meas}/stretch2_up_{angle}.png", dpi=500)
    fig4 = RelativeStretchGraph2(df)
    fig4.savefig(f"./VCO_stretch/figures/{meas}/stretch2_up_{angle}_rel.png", dpi=500)
    plt.show()
